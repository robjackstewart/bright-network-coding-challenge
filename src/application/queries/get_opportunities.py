...
class GetOpportunitiesQuery:
    ...
    async def execute(self) -> dict[Member, set[Job]]:
        ...
        for member in members:
            ...
            meaningful_bio_words = [
                word
                for word in GetOpportunitiesQuery.__strip_non_alphanumeric_chars(
                    lower_bio
                ).split()
                if len(word) > 3
            ]
            preferred_geographies: list[str] = (
                GetOpportunitiesQuery.__calculate_preferred_geographies(
                    lower_bio, locations
                )
            )
            ...
            for job in jobs_to_consider_for_member_based_on_geography:
                ...
                for word in meaningful_bio_words:
                    if (
                        word not in locations
                        and word in lower_case_job_title
                        and job not in jobs_for_member
                    ):
    ...
    def __calculate_preferred_geographies(
        bio: str, available_geographies: list[str] = []
    ) -> Optional[list[str]]:
        locations_in_bio = [
